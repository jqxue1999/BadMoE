...
\textbf{Token positions.} The two directions are read at different sequence positions, reflecting their different roles. The input-risk direction $\drefuse$ is computed at the prompt's last token, immediately before generation begins. The output-safety direction $\dbeh$ is computed by averaging per-token residuals across the response, since the contrast captures generation tendency rather than input perception. Both are unit-normalized per layer.

\subsection{Online Stage: Per-Query Identification and Modulation}
\label{sec:online}

Both challenges are met by a single forward-backward pass on the input: from one mid-layer probe we read the modulation signal directly, and the per-expert identification gradient is obtained by one shared backward through the same probe, regardless of how many experts are scored. Concretely, for each query $q$, we forward through layers $\{0, \ldots, \ell^*\}$ and read the residual at the prompt's last token, $h(q) = \mathbf{h}_{\ell^*, T(q)}(q)$. From this single state, two scalars are obtained by projection onto the offline directions (Figure~\ref{fig:method}~(c)):
\begin{align}
