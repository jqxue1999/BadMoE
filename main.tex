...
\caption{Adaptive Router Alignment. Top-left: the input-risk direction $\drefuse$ is the offline mean-diff between residuals on risky and benign prompts. Top-right: the output-safety direction $\dbeh$ is the offline mean-diff between residuals on safe and harmful continuations of the same risky prompts. Bottom: at inference, one mid-layer probe $h(q)$ is read for each query; its projections onto the two directions yield the risky index $\lambda(q) = h(q) \cdot \drefuse$ and the refusal score $s(q) = h(q) \cdot \dbeh$. The gradient $\nabla_{\ell,e}\, s(q)$ ranks per-(layer, expert) attribution to refusal, $\lambda(q)$ scales the resulting router-bias adjustment $\Delta b$, and benign inputs land at $\lambda(q) \approx 0$ so the bias collapses to zero and routing is left unchanged.}
\label{fig:method}
\end{figure}
...
Routing is the lever. Because an MoE forward pass commits the per-token compute budget through one router decision at each layer, the most direct way to reshape the model's computation on a given input is to perturb its router logits before the top-$k$ selection. Adaptive Router Alignment uses exactly this lever and leaves model weights untouched.
...
\begin{equation}
  ...
  \label{eq:dsafe}
\end{equation}
The projection $h(q) \cdot \dbeh[\ell]$ measures, in the model's own representation space, how strongly the rollout from this state pushes toward a refusal completion. Because the contrast was learned on harmful inputs only, this projection is meaningful in the harmful-input regime, which is exactly where the defense uses it. Both directions are unit-normalized per layer and stored once; nothing in the offline stage is repeated at inference.

\subsection{Online Stage: Per-Query Identification and Modulation}
\label{sec:online}

For each query $q$, we forward through layers $\{0, \ldots, \ell^*\}$ and read the residual at the prompt's last token, $h(q) = \mathbf{h}_{\ell^*, T(q)}(q)$. From this single state, two scalars are read by projection onto the offline directions (Figure~\ref{fig:method}, bottom):
\begin{align}
  \lambda(q) &= h(q) \cdot \drefuse[\ell^*] && \text{(risky index)}, \label{eq:lambda} \\
  s(q) &= h(q) \cdot \dbeh[\ell^*] && \text{(refusal score)}. \label{eq:srefuse}
\end{align}
$\lambda(q)$ is the modulation signal; $s(q)$ is the source from which the per-expert identification gradient is taken. Because both come from the same forward pass, the online cost is one shared forward plus one shared backward, regardless of how many experts are scored.
...
The bias on expert $(\ell,e)$ on query $q$ is the product of three factors: a global strength $\beta$, a query-level modulation $\rho(\lambda(q)) \in [0,1]$ that ramps from off to full on as the risky index grows, and a per-expert weight proportional to relative attribution magnitude:
\begin{equation}
  b_{\ell,e}(q) = -\,\beta \cdot \rho\!\bigl(\lambda(q)\bigr) \cdot \frac{|\Delta_{\ell,e}(q)|}{\max_{(\ell',e') \in \Amin(q)} |\Delta_{\ell',e'}(q)|}
  \quad \text{for } (\ell,e) \in \Amin(q),
  \label{eq:bias}
\end{equation}
and $b_{\ell,e}(q) = 0$ otherwise. The query-level ramp is a clipped linear function of $\lambda(q)$,
\begin{equation}
  \rho(\lambda) = \operatorname{clip}\!\left(\frac{\lambda - \lambda_{\text{lo}}}{\lambda_{\text{hi}} - \lambda_{\text{lo}}},\; 0,\; 1\right),
  \label{eq:ramp}
\end{equation}
with $\lambda_{\text{lo}}$ and $\lambda_{\text{hi}}$ calibrated once from held-out distributions ($95$th percentile of $\lambda$ on a benign mixture and $5$th percentile on harmful queries, respectively); no per-attack or per-deployment fitting is required. Truly benign queries land below $\lambda_{\text{lo}}$ and receive $\rho = 0$, so the entire bias collapses to zero and the model's normal routing is preserved. Clearly harmful queries land above $\lambda_{\text{hi}}$ and receive the full bias on their per-query risk-amplifying experts. Ambiguous queries interpolate between the two. This is the route in Figure~\ref{fig:method} (bottom) that splits ``risky inputs: suppress risky experts'' from ``normal inputs: no intervention''.
