"""
Stage L41: L28 reproduction under SteerMoE-exact generation params.

L28 (the 90% winner) was run with max_new_tokens=150, which may have cut
off multi-paragraph harmful responses mid-stream and biased the judge.
SteerMoE paper uses max_new_tokens=512 (from demo.ipynb: temperature=0.0,
top_p=1, top_k=1, max_tokens=512, seed=0). Since our generate() uses
do_sample=False (equivalent to temperature=0 → deterministic greedy),
top_p / top_k values are moot. So matching SteerMoE only requires
matching max_new_tokens, which L41 does (150→512). Judge response
truncation also widened to 3500 chars.

This gives the FIRST directly-comparable L28 vs SteerMoE-orig datapoint:
  * L40 steermoe_orig = 55% direct / 80% benign (at max=512)
  * L41 L28 d=1.0 = ??? / ??? (new, at max=512)

Headline enables: "our L28 achieves X% direct / Y% benign where SteerMoE
reaches 55% / 80% on the same generation protocol."

All other code below is verbatim from L28; only max_new_tokens and judge
response truncation are changed (150→512, 1000→2500 chars).

Original L28 docstring follows:
=============================================
Stacked intervention — global A+ saturation + per-query soft bump.
...
OUT_CSV = f"{DATA_DIR}/stage_l41_l28_max512.csv"
...
def gen_response(model, tok, q, max_new=512):   # L41: 150 → 512 (SteerMoE-exact)
    msgs = [{"role": "user", "content": q}]
    text = tok.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
    inputs = tok(text, return_tensors="pt").to(model.device)
    with torch.no_grad():
...
def judge_file(path, tok_j, model_j):
    ...
    for i, r in enumerate(rows):
        if r.get("score") not in ("", None):
            continue
        # L41: 1000 → 3500 chars to accommodate 512-token responses
        # (512 tokens × 3-5 chars/token English → up to ~3000 chars; 3500 covers
        # long-tail outputs without blowing judge context budget).
        text = JUDGE_PROMPT.format(question=r["q"][:300], response=r["resp"][:3500])
