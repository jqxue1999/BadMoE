"""
Stage L22 = Ablation A2: v_e used as SCALAR projection (query-level, not per-token).

Isolates the effect of replacing L9's static scalar cap with a query-dependent
scalar derived from v_e · h_last_prompt. Formula:
    λ(q)        = ReLU(h_probe · d_refuse_unit)       scalar per query
    scalar_e^ℓ  = ⟨h^ℓ_last_prompt, v_e^ℓ_unit⟩        per (layer, expert)  — QUERY-LEVEL
    bias_e      = α · λ(q) · scalar_e                  broadcast to all tokens
    gate_logits += bias

Compared to L20 (A3, per-token bilinear):
  • L22: scalar_e computed ONCE from last prompt token; same for all decode tokens
  • L20: ⟨h(t), v_e⟩ recomputed per token

Compared to L21 (A1, static cap):
  • L22: scalar_e is query-dependent (via v_e · h)
  • L21: cap_e is query-independent static (||d · W_down||)

If L22 matches L21 (both better or both worse than L20): per-token dynamics hurts.
If L22 matches L20 (both bad): the v_e replacement is the real problem.
If L22 is the best: v_e-scalar gives gain over static cap; but per-token adds noise.
"""
...
OUT_CSV = f"{DATA_DIR}/stage_l22_v_e_scalar.csv"
...
class PECRBScalarHooks:
    """PECRB with QUERY-LEVEL scalar v_e projection (A2 ablation).

    Compared to L20 (A3, per-token bilinear), here scalar_e is computed ONCE
    from the last prompt token of each bias layer (captured on prefill) and
    reused for all decode tokens — closer in form to L9's static cap but with
    a query-dependent metric.

    bias_e = α · λ(q) · ⟨h^ℓ_last_prompt, v_e_unit^ℓ⟩   [n_experts]
    gate_logits += bias  (broadcast to every token)
    """
    ...
    def __init__(self, model, v_e, bias_layers, alpha, d_refuse, probe_layer):
        ...
        self._lam = None
        self._scalar_bias = {}   # li -> [n_experts] query-level scalar
        self._bias_store = {}
        self._handles = []
    ...
    def _make_mlp_pre(self, li):
        def pre(module, args):
            ...
            # On prefill (multi-token), freeze the QUERY-LEVEL scalar from last prompt token.
            if n_tokens > 1:
                h_last = h_flat[-1]                 # [hidden]
                scalar_e = v @ h_last               # [n_experts]
                self._scalar_bias[li] = scalar_e

            scalar_e = self._scalar_bias.get(li, None)
            if scalar_e is None:
                # Defensive: should never happen (probe fires on prefill before this)
                return
            if self._lam is None:
                lam_scalar = 0.0
            else:
                lam_scalar = float(self._lam[0].item())
            scale = self.alpha * lam_scalar
            # Broadcast [n_experts] → [N_tokens, n_experts]
            bias = scale * scalar_e.to(device=h_flat.device, dtype=torch.float32).unsqueeze(0).expand(n_tokens, -1)
...
def run_condition(model, tok, prompts, tag, v_e=None, alpha=0.0,
    ...
    if v_e is None or alpha == 0.0:
        for q in prompts:
            ...
            torch.cuda.empty_cache()
        return rows
    with PECRBScalarHooks(model, v_e=v_e, bias_layers=BIAS_LAYERS, alpha=alpha,
...
def main() -> None:
    ...
    # A2: bias_e = α · λ(q) · ⟨h_last_prompt, v_e_unit⟩ (query-level scalar, broadcast)
    # α sweep wider than L20 because v_e is unit-normalized and ||h|| adds ~5-20 scale,
    # so scalar_e is roughly cos-like with magnitude ~1-8. α ≈ 1-30 likely sweet.
    CONDITIONS = [
        ("baseline", 0.0, None),
        ("pecrb_scalar_a+0.3", +0.3, v_e_dict),
        ("pecrb_scalar_a+1", +1.0, v_e_dict),
        ("pecrb_scalar_a+3", +3.0, v_e_dict),
        ("pecrb_scalar_a+10", +10.0, v_e_dict),
        ("pecrb_scalar_a+30", +30.0, v_e_dict),
        ("pecrb_scalar_a-3_sanity", -3.0, v_e_dict),
    ]

    subsets = [("direct", DIRECT_HARMFUL), ("xstest", XSTEST_LIKE), ("benign", BENIGN)]

    print("\n" + "=" * 60, flush=True)
    print("Phase 2: Generation (v_e-scalar gated, A2)", flush=True)
    print("=" * 60, flush=True)
    for cond_tag, alpha, v_e_in in CONDITIONS:
        print(f"\n--- {cond_tag} ---", flush=True)
        for subset_name, prompts in subsets:
            rows = run_condition(model, tok, prompts, tag=f"{cond_tag}/{subset_name}",
                                 v_e=v_e_in, alpha=alpha,
                                 d_refuse=d_refuse, probe_layer=PROBE_LAYER)
