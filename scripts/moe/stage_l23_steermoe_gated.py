"""
Stage L23 = Ablation S1: SteerMoE + λ(q) query-level gate.

Starting from L15's VERIFIED-WORKING SteerMoE (A+25 gave 70→85% on OLMoE),
flip ONE axis: add a λ(q) gate so the saturation intervention only fires on
queries the probe flags as harmful-like (λ>threshold). Benign queries are
left exactly as OLMoE baseline would produce.

Design:
  λ(q)      = ReLU(h_probe · d_refuse_unit)       ONCE per query (prefill)
  Saturation applied iff λ(q) > λ_threshold
  Otherwise return raw gate output unchanged → SteerMoE-free pass

Reuses L15's RD (A⁺ = top-25 by |Δ|) already saved at stage_l15_rd.pt.

Expected outcomes:
  - direct safe% ≈ L15 85% (harmful queries fire gate just like SteerMoE)
  - xstest safe% ≥ L15 (gate avoids intervention on borderline that λ→0)
  - benign safe% ≈ 100% ≫ L15 86.7% (benign queries bypass intervention entirely)

If this works, we've shown λ gate is a pure benign-safety win on top of SteerMoE.
Then next steps (L24+) swap operator/metric/selection one at a time.
"""
...
OUT_CSV = f"{DATA_DIR}/stage_l23_steermoe_gated.csv"
...
class SteerMoEGatedHooks:
    """SteerMoE log-softmax saturation GATED by λ(q).
    ...
    Flow:
      ...
      2. At each bias layer's gate: if λ(q) ≤ λ_threshold → return raw output
         unchanged (SteerMoE-free). If λ > threshold → apply SteerMoE's full
         ε-saturation (the same operator as L15).

    This is Step S1 in the SteerMoE → PECRB ablation chain: introduce ONLY
    the query gate, keep all other SteerMoE mechanics exactly.
    """
...
def run_condition(model, tok, prompts, tag, A_plus=None, A_minus=None, eps=EPS,
    ...
    with SteerMoEGatedHooks(model, A_plus or {}, A_minus or {}, epsilon=eps,
        ...
        for q in prompts:
            # Critical: clear stale λ from previous query, then warm up for THIS
            # query so pre-probe bias layers get the correct λ on first prefill.
            hooks._lam = None
...
def main() -> None:
    ...
    # Focus on L15's sweet-spot A+25 (best SteerMoE = 85%). Sweep λ threshold.
    # λ_threshold: 0.0 = ReLU-only (any harm signal → gate fires);
    #              higher = stricter (only very harmful queries → gate).
    # (tag, n_plus, n_minus, eps, lam_threshold)
    INTERVENTIONS = [
        ("baseline", 0, 0, EPS, 0.0),
        ("smoe_A+25_ungated",       25, 0, 0.01, -1e9),   # ungated = reproduce L15 A+25 (85% expected)
        ("smoe_A+25_gated_th0.0",   25, 0, 0.01, 0.0),
        ("smoe_A+25_gated_th0.5",   25, 0, 0.01, 0.5),
        ("smoe_A+25_gated_th1.0",   25, 0, 0.01, 1.0),
        ("smoe_A+5_gated_th0.0",     5, 0, 0.01, 0.0),    # smaller A+ for xstest comparison
        ("smoe_A+10_gated_th0.0",   10, 0, 0.01, 0.0),
    ]
    ...
    for cond_tag, n_plus, n_minus, eps, lam_th in INTERVENTIONS:
