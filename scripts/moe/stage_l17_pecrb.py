"""
Stage L17: PECRB (Per-Expert Contrastive Router Bias) — main method experiment on OLMoE.

Online: for each token t at each bias layer ℓ,
   bias_e^ℓ(t) = α · ⟨h^ℓ(t), v_e^ℓ⟩         (α < 0)
   gate_logits_e^ℓ(t) += bias_e^ℓ(t)

v_e^ℓ is loaded from v_e_method_B.pt (contrastive mean-diff extractor).

Conditions (7):
  baseline                    no intervention
  pecrb_alpha=-3              full bilinear (boost + suppress)
  pecrb_alpha=-10
  pecrb_alpha=-20
  pecrb_suppress_only (α=-10) clamp positive biases to 0 (only downward)
  pecrb_boost_only    (α=-10) clamp negative biases to 0 (only upward)
  l9_cap_static (α=-10)       single-direction baseline: α · λ(q) · (cap - mean)

Benchmarks:
  DIRECT_HARMFUL (20)   direct harmful requests (baseline safety test)
  XSTEST_LIKE    (20)   borderline-benign prompts (over-refusal test)
  BENIGN         (15)   unambiguous benign (quality preservation)

Judge: Qwen2.5-14B-Instruct (alignment 0-100).
Incremental CSV save after every condition × prompt subset (timeout-safe).
"""
...
OUT_CSV = f"{DATA_DIR}/stage_l17_pecrb.csv"
SEED = 42
...
def main() -> None:
    ...
    # α sign convention:
    #   PECRB uses α < 0 (high ⟨h, v_e_unit⟩ = unsafe → bias < 0 to suppress)
    #   L9 cap_static uses α > 0 (high (cap_e − mean) = more safe-capable → bias > 0 to boost)
    # After v_e unit-normalisation, effective bias ≈ α · ||h|| · cos(h, v_e).
    # For OLMoE ||h|| ~10–20, cos ~0.1–0.4, so bias scale ≈ α · 1–8.
    # Gate logit natural scale ~5, so α ∈ {−0.3, −1, −3, −10} spans subtle → strong.
    CONDITIONS = [
        ("baseline", 0.0, None, "full", None),
        ("pecrb_a-0.3", -0.3, v_e_dict, "full", None),
        ("pecrb_a-1", -1.0, v_e_dict, "full", None),
        ("pecrb_a-3", -3.0, v_e_dict, "full", None),
        ("pecrb_a-10", -10.0, v_e_dict, "full", None),
        ("pecrb_sup_only_a-3", -3.0, v_e_dict, "suppress_only", None),
        ("pecrb_boo_only_a-3", -3.0, v_e_dict, "boost_only", None),
        ("l9_cap_static_a+10", +10.0, None, "cap_static", cap_refuse),
    ]
