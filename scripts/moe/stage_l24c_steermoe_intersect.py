"""
Stage L24c: Dynamic A⁺ = static_A⁺ ∩ router's natural top-k + λ gate.

Test "per-query expert selection helps" hypothesis. Starting from L23
(static A+25 SteerMoE + λ gate), flip ONE axis: at EACH TOKEN, compute
router's NATURAL top-8 on raw logits, intersect with the static A+=25 set,
and only saturate the intersection.

Design:
  λ(q)  = ReLU(h_probe · d_refuse_unit)
  A+ (static, from L15's RD)  = top-25 by |Δ| globally
  Inside each gate hook:
    if λ(q) > τ:
        natural_top_k = argsort(softmax(raw_logits))[-top_k:]
        dynamic_A+    = static_A+ ∩ natural_top_k  (may be empty)
        apply SteerMoE saturation ONLY on dynamic_A+
    else: pass-through (benign)

Kill condition: L24c ≥ L23 direct safe% (at benign ≥98%) — otherwise per-query
selection framing is not supported on OLMoE.

Also includes two control conditions:
  random_K25: dynamic A+ = random 25 experts (noise floor)
  l23_repro:  same as L23 A+25_gated_th0 (sanity anchor)
"""
...
OUT_CSV = f"{DATA_DIR}/stage_l24c_steermoe_intersect.csv"
RD_PATH = f"{DATA_DIR}/stage_l15_rd.pt"           # REUSE L15's RD extraction (A+ set)
D_REFUSE_PATH = f"{DATA_DIR}/d_refuse.pt"         # for λ(q) probe
PROBE_LAYER = 10
SEED = 42
EPS = 0.01
...
class SteerMoEGatedHooks:
    """SteerMoE log-softmax saturation GATED by λ(q), with DYNAMIC A⁺ intersection.

    Flow:
      1. At probe_layer residual output, on prefill only, compute
         λ(q) = ReLU(h_last_prompt · d_refuse_unit). Store.
      2. At each bias layer's gate: if λ(q) ≤ λ_threshold → return raw output
         unchanged (benign pass-through).
      3. If λ > threshold: compute router's NATURAL top-k per token from the
         raw logits, then intersect with the static A+/A- sets. Only
         saturate the intersection (dynamic_A+, dynamic_A-).
         When intersect_mode="none", behaves like L23 (static A+ saturated
         regardless of router's natural choice).

    intersect_mode ∈ {"none", "natural_topk"}:
      - "none"          → L23 behavior (static saturation on entire A+)
      - "natural_topk"  → L24c behavior (saturate only static_A+ ∩ router_top_k)
    """

    def __init__(self, model, A_plus, A_minus, epsilon, d_refuse, probe_layer,
                 lam_threshold=0.0, intersect_mode="none"):
        assert intersect_mode in {"none", "natural_topk"}
        self.model = model
        self.A_plus = A_plus
        ...
        self._lam = None  # scalar tensor [B], set on prefill of each query
        self._handles = []

    def _make_probe_hook(self):
        """Capture λ(q) on prefill from last prompt token; hold through decode."""
        def post(module, inp, out):
            ...
            self._lam = torch.clamp((last @ d_u), min=0.0)  # [B]
        return post
    ...
    def _make_hook(self, li):
        ...
        def hook(module, inp, out):
            # λ-gate: skip intervention when query isn't flagged harmful.
            if self._lam is None:
                return out
            lam_scalar = float(self._lam[0].item())  # batch=1 assumed
            if lam_scalar <= self.lam_threshold:
                return out   # benign pass-through → identical to L15 baseline

            # ---------- Identical to L15 SteerMoE saturation below ----------
            ...
            if self.intersect_mode == "natural_topk":
                # Compute router's natural top-k per token (from raw logits).
                top_k = getattr(module, "top_k", 8)
                natural_topk = torch.topk(raw, top_k, dim=-1).indices  # [N, top_k]
                # Build a boolean mask over [N, n_experts] for the static A+/A-,
                # then AND with natural_topk_mask to get the dynamic intersection.
                N, E = raw.shape
                if plus_t is not None:
                    plus_mask = torch.zeros(E, dtype=torch.bool, device=s.device)
                    plus_mask[plus_t.to(s.device)] = True
                    # per-token natural mask
                    natural_mask = torch.zeros(N, E, dtype=torch.bool, device=s.device)
                    natural_mask.scatter_(1, natural_topk.to(s.device), True)
                    dyn_plus_mask = natural_mask & plus_mask.unsqueeze(0)  # [N, E]
                    s_fill = (s_max + self.epsilon).expand(-1, E)
                    s = torch.where(dyn_plus_mask, s_fill, s)
                if minus_t is not None:
                    minus_mask = torch.zeros(E, dtype=torch.bool, device=s.device)
                    minus_mask[minus_t.to(s.device)] = True
                    natural_mask = torch.zeros(N, E, dtype=torch.bool, device=s.device)
                    natural_mask.scatter_(1, natural_topk.to(s.device), True)
                    dyn_minus_mask = natural_mask & minus_mask.unsqueeze(0)
                    s_fill = (s_min - self.epsilon).expand(-1, E)
                    s = torch.where(dyn_minus_mask, s_fill, s)
            else:
                # "none" — static saturation (L23 baseline behavior).
                if plus_t is not None:
...
def warmup_lambda(model, tok, q):
    """Run a prompt-only forward so the probe hook computes λ(q) BEFORE the
    actual generate() prefill. This guarantees λ is set for ALL bias layers,
    including those below probe_layer. The subsequent generate() will refire
    the probe and recompute the same λ (h_last_prompt is identical), so this
    is semantically idempotent — only a ~10% compute overhead per query."""
    msgs = [{"role": "user", "content": q}]
    text = tok.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
    inputs = tok(text, return_tensors="pt").to(model.device)
    with torch.no_grad():
...
def run_condition(model, tok, prompts, tag, A_plus=None, A_minus=None, eps=EPS,
                  d_refuse=None, probe_layer=PROBE_LAYER, lam_threshold=0.0,
                  intersect_mode="none"):
    rows = []
    if A_plus is None and A_minus is None:
        for q in prompts:
            ...
            torch.cuda.empty_cache()
        return rows
    with SteerMoEGatedHooks(model, A_plus or {}, A_minus or {}, epsilon=eps,
                            d_refuse=d_refuse, probe_layer=probe_layer,
                            lam_threshold=lam_threshold,
                            intersect_mode=intersect_mode) as hooks:
        for q in prompts:
            hooks._lam = None
...
def main() -> None:
    ...
    # ---------- Phase 1: REUSE L15's RD + load d_refuse ----------
    print("\n" + "=" * 60, flush=True)
    print("Phase 1: Load precomputed RD (from L15) and d_refuse", flush=True)
    print("=" * 60, flush=True)
    if not os.path.exists(RD_PATH):
        raise FileNotFoundError(
            f"RD not found at {RD_PATH}. Run L15 first to extract RD.")
    if not os.path.exists(D_REFUSE_PATH):
        raise FileNotFoundError(f"d_refuse not found at {D_REFUSE_PATH}.")
    rd_obj = torch.load(RD_PATH, map_location="cpu", weights_only=False)
    rd = rd_obj["rd"]
    d_refuse = torch.load(D_REFUSE_PATH, map_location="cpu", weights_only=False)
    print(f"  rd shape {tuple(rd.shape)}  d_refuse shape {tuple(d_refuse.shape)}",
          flush=True)
    print(f"  RD stats: min={rd.min():.5f}, max={rd.max():.5f}, "
          f"abs-max={rd.abs().max():.5f}", flush=True)

    # ---------- Phase 2: Intervention conditions ----------
    # Test dynamic A+ (natural_topk intersection) vs static A+ (L23 behavior).
    # All share λ gate (th=0.0) and A+25 (from L15). Only intersect_mode varies.
    # (tag, n_plus, n_minus, eps, lam_threshold, intersect_mode)
    INTERVENTIONS = [
        ("baseline",                       0, 0, EPS,   0.0, "none"),
        # Anchor: reproduce L23 behavior (static A+ saturation, λ gate).
        ("l23_static_A+25_gated_th0.0",   25, 0, 0.01,  0.0, "none"),
        # L24c main: dynamic A+ = static A+ ∩ natural top-k (intersect_mode="natural_topk").
        ("l24c_dynamic_A+25_gated_th0.0", 25, 0, 0.01,  0.0, "natural_topk"),
        # Vary λ threshold under dynamic selection.
        ("l24c_dynamic_A+25_gated_th0.5", 25, 0, 0.01,  0.5, "natural_topk"),
        ("l24c_dynamic_A+25_gated_th1.0", 25, 0, 0.01,  1.0, "natural_topk"),
        # Smaller A+ size (dynamic).
        ("l24c_dynamic_A+10_gated_th0.0", 10, 0, 0.01,  0.0, "natural_topk"),
        ("l24c_dynamic_A+5_gated_th0.0",   5, 0, 0.01,  0.0, "natural_topk"),
    ]

    subsets = [("direct", DIRECT_HARMFUL), ("xstest", XSTEST_LIKE), ("benign", BENIGN)]

    print("\n" + "=" * 60, flush=True)
    print("Phase 2: Generation under λ-gated SteerMoE", flush=True)
    print("=" * 60, flush=True)

    for cond_tag, n_plus, n_minus, eps, lam_th, intersect_mode in INTERVENTIONS:
        if n_plus == 0 and n_minus == 0:
            ...
            print(f"\n--- {cond_tag} (|A+|={ap} |A-|={am} ε={eps} λ_th={lam_th} "
                  f"mode={intersect_mode}) ---", flush=True)
        for subset_name, prompts in subsets:
            rows = run_condition(
                model, tok, prompts, tag=f"{cond_tag}/{subset_name}",
                A_plus=A_plus, A_minus=A_minus, eps=eps,
                d_refuse=d_refuse, probe_layer=PROBE_LAYER,
                lam_threshold=lam_th, intersect_mode=intersect_mode,
            )
