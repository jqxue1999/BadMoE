"""
Stage L18: PECRB v2 with response-conditioned v_e (SteerMoE-style contrastive).

Problem diagnosed in L17 / L17b:
  Our original v_e = E[h | e fires on harmful prompt] − E[h | e fires on benign prompt].
  For OLMoE with 70% baseline refusal rate, the harmful-prompt signal is dominated by
  refusal-generation expert recruitment. v_e is thus a 70/30 weighted mixture of
  refuse-enablers + harm-generators. Sign of ⟨h, v_e⟩ is uninterpretable.

Fix (this script):
  Contrast on RESPONSE tokens of matched harmful prompts, mimicking SteerMoE Eq. 12:
    x_refuse  = harmful_prompt + canned_refusal_template
    x_comply  = harmful_prompt + compliance_template
    v_e[l, e] = E[h(t) | e fires on t, t ∈ REFUSE response]
              − E[h(t) | e fires on t, t ∈ COMPLY response]

  Then at inference on harmful q:
    bias_e(q) = α · ⟨h(q), v_e⟩       (α > 0)
  Experts with large positive ⟨h, v_e⟩ at refuse-leaning h(q) are refuse-writers;
  boosting them amplifies the refuse trajectory.

Phases:
  1. Extract v_e_clean on 30 training (harmful_prompt, unsafe_template) pairs.
  2. Save v_e_clean to disk.
  3. Generate under PECRB (α > 0) on same prompts as L17 for head-to-head.
  4. Qwen2.5-14B judge.
"""
...
OUT_CSV = f"{DATA_DIR}/stage_l18_pecrb_clean.csv"
...
class PECRBHooks:
    def __init__(self, model, v_e, bias_layers, alpha, mode="full"):
...
def run_condition(model, tok, prompts, tag, v_e=None, alpha=0.0, mode="full"):
    ...
    if v_e is None or alpha == 0.0:
        for q in prompts:
            ...
            torch.cuda.empty_cache()
        return rows
    with PECRBHooks(model, v_e=v_e, bias_layers=BIAS_LAYERS, alpha=alpha, mode=mode):
        for q in prompts:
            resp = gen_response(model, tok, q)
            rows.append({"tag": tag, "q": q, "resp": resp})
            torch.cuda.empty_cache()
...
def main() -> None:
    ...
    # ---------- Phase 1: extract v_e_clean ----------
    print("\n" + "=" * 60, flush=True)
    print("Phase 1: Extract v_e_clean (response-conditioned)", flush=True)
    print("=" * 60, flush=True)
    training_pairs = [
        (prompt, UNSAFE_TEMPLATES[i % len(UNSAFE_TEMPLATES)])
        for i, prompt in enumerate(HARMFUL_TRAIN)
    ]
    print(f"Training pairs: {len(training_pairs)}", flush=True)

    v_e, cnt_refuse, cnt_comply = extract_v_e_clean(model, tok, training_pairs)
    torch.save({
        "v_e": v_e,
        "cnt_refuse": cnt_refuse,
        "cnt_comply": cnt_comply,
        "n_training_pairs": len(training_pairs),
        "note": "v_e[l,e] = mean(h | e fires on refuse response token) - mean(h | e fires on comply response token).",
    }, VE_PATH)
    print(f"Saved v_e_clean → {VE_PATH}", flush=True)
    for li in BIAS_LAYERS:
        norms = v_e[li].norm(dim=-1)
        print(f"  L{li}: ||v_e|| mean={norms.mean():.4f} std={norms.std():.4f} "
              f"min={norms.min():.4f} max={norms.max():.4f} | "
              f"cnt_refuse avg={cnt_refuse[li].float().mean():.1f} "
              f"cnt_comply avg={cnt_comply[li].float().mean():.1f}", flush=True)

    # Pack dict for BIAS_LAYERS only
    v_e_dict = {li: v_e[li] for li in BIAS_LAYERS}

    # ---------- Phase 2: conditions ----------
    # α > 0: BOOST experts aligned with (refuse − comply) direction on harmful queries.
    CONDITIONS = [
        ("baseline", 0.0, None, "full"),
        ("pecrb_clean_a+0.3", +0.3, v_e_dict, "full"),
        ("pecrb_clean_a+1", +1.0, v_e_dict, "full"),
        ("pecrb_clean_a+3", +3.0, v_e_dict, "full"),
        ("pecrb_clean_boo_only_a+1", +1.0, v_e_dict, "boost_only"),
        # Sign-flip sanity: α < 0 should HURT safety if v_e is correctly refuse-writer vector.
        ("pecrb_clean_a-1", -1.0, v_e_dict, "full"),
    ]
