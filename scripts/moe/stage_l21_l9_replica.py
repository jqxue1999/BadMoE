"""
Stage L21 = Ablation A1: L9 original formula, ported UNCHANGED to OLMoE.

Formula (identical to Qwen3-Next L9 that worked):
    λ(q)    = ReLU(h_probe · d_refuse_unit)      scalar per query
    bias_e  = α · λ(q) · (cap_e − mean_cap)      static scalar per expert
    gate_logits += bias

Compared to L20 (which changed TWO things vs L9: added v_e vector + per-token
bilinear), L21 changes ZERO things. It answers the kill-shot question:
  "Does L9's exact formula transfer to OLMoE?"

If L21 ≥ 80% direct safe:
  → L9 formula transfers; failure of L20/L18 is caused by our v_e addition.
If L21 ≈ baseline 70%:
  → OLMoE architecture itself doesn't respond to α·λ·(cap-mean) biasing;
    any PECRB iteration has architectural ceiling.
If L21 < 70%:
  → L9 formula actively hurts OLMoE; SteerMoE's log-softmax saturation is
    the only operator that works on this architecture.

Inputs:
  - d_refuse.pt (already exists)
  - cap_refuse.pt (already exists from L12/L16)
"""
...
OUT_CSV = f"{DATA_DIR}/stage_l21_l9_replica.csv"
CAP_PATH = f"{DATA_DIR}/cap_refuse.pt"
...
class L9CapStaticHooks:
    """L9-exact formula ported to OLMoE: α · λ(q) · (cap_e − mean_cap).

    λ(q) = ReLU(h_probe · d_refuse_unit) computed from LAST PROMPT TOKEN once
    per query. cap_e is STATIC per expert (from ‖d_refuse · W_down,e‖); its
    value is query-independent.

    bias_e(t) = α · λ(q) · (cap_e − mean_cap_layer)     broadcast to all tokens
    """

    def __init__(self, model, cap_refuse, bias_layers, alpha, d_refuse, probe_layer):
        self.model = model
        self.bias_layers = bias_layers
        self.alpha = alpha
        # Precompute centered cap per layer: (cap_e - mean_cap_layer)
        self.cap_centered = {}
        for li in bias_layers:
            c = cap_refuse[li].float()
            self.cap_centered[li] = c - c.mean()
        # λ-probe setup
    ...
    def _make_mlp_pre(self, li):
        def pre(module, args):
            ...
            cap_c = self.cap_centered[li].to(device=h_flat.device, dtype=torch.float32)
            # L9 formula: bias = α · λ(q) · (cap_e − mean)
            # cap_c is [n_experts]; broadcast to [N_tokens, n_experts].
            if self._lam is None:
                lam_scalar = 0.0
            else:
                lam_scalar = float(self._lam[0].item())  # batch=1 assumed
            n_tokens = h_flat.shape[0]
            bias = (self.alpha * lam_scalar) * cap_c.unsqueeze(0).expand(n_tokens, -1)
...
def run_condition(model, tok, prompts, tag, cap_refuse=None, alpha=0.0,
                  d_refuse=None, probe_layer=PROBE_LAYER):
    rows = []
    if cap_refuse is None or alpha == 0.0:
        for q in prompts:
            resp = gen_response(model, tok, q)
            rows.append({"tag": tag, "q": q, "resp": resp})
            torch.cuda.empty_cache()
        return rows
    with L9CapStaticHooks(model, cap_refuse=cap_refuse, bias_layers=BIAS_LAYERS,
                          alpha=alpha, d_refuse=d_refuse, probe_layer=probe_layer):
        for q in prompts:
            resp = gen_response(model, tok, q)
            rows.append({"tag": tag, "q": q, "resp": resp})
            torch.cuda.empty_cache()
...
def main() -> None:
    ...
    # ---------- Phase 1: Load cap_refuse + d_refuse ----------
    print("\n" + "=" * 60, flush=True)
    print("Phase 1: Load cap_refuse + d_refuse (L9-style scalar per-expert capacity)", flush=True)
    print("=" * 60, flush=True)
    if not os.path.exists(CAP_PATH):
        raise FileNotFoundError(f"cap_refuse.pt not found at {CAP_PATH}.")
    if not os.path.exists(D_REFUSE_PATH):
        raise FileNotFoundError(f"d_refuse.pt not found at {D_REFUSE_PATH}.")
    cap_refuse = torch.load(CAP_PATH, map_location="cpu", weights_only=False)
    d_refuse = torch.load(D_REFUSE_PATH, map_location="cpu", weights_only=False)
    print(f"  cap_refuse shape {tuple(cap_refuse.shape)}, "
          f"d_refuse shape {tuple(d_refuse.shape)}", flush=True)
    for li in BIAS_LAYERS:
        c = cap_refuse[li].float()
        print(f"  L{li}: cap mean={c.mean():.3f} std={c.std():.3f} "
              f"min={c.min():.3f} max={c.max():.3f}", flush=True)

    # ---------- Phase 2: conditions ----------
    # A1 = L9's exact formula: bias_e = α · λ(q) · (cap_e − mean_cap)
    # Greedy decoding → baseline is deterministic.
    # α range: L9 on Qwen3-Next used α=10; here we sweep to find OLMoE's sweet spot.
    CONDITIONS = [
        ("baseline", 0.0),
        ("l9_cap_gated_a+1", +1.0),
        ("l9_cap_gated_a+3", +3.0),
        ("l9_cap_gated_a+10", +10.0),     # L9's Qwen3 value
        ("l9_cap_gated_a+30", +30.0),
        ("l9_cap_gated_a+100", +100.0),   # aggressive — cap values are small
        ("l9_cap_gated_a-10_sanity", -10.0),   # sanity flip
    ]

    subsets = [("direct", DIRECT_HARMFUL), ("xstest", XSTEST_LIKE), ("benign", BENIGN)]

    print("\n" + "=" * 60, flush=True)
    print("Phase 2: Generation (L9 replica α·λ·(cap−mean))", flush=True)
    print("=" * 60, flush=True)
    for cond_tag, alpha in CONDITIONS:
        print(f"\n--- {cond_tag} ---", flush=True)
        for subset_name, prompts in subsets:
            rows = run_condition(
                model, tok, prompts, tag=f"{cond_tag}/{subset_name}",
                cap_refuse=(cap_refuse if alpha != 0.0 else None),
                alpha=alpha, d_refuse=d_refuse, probe_layer=PROBE_LAYER,
            )
