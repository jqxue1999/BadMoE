...
A natural symmetric variant of \name amplifies $\Apos(q)$, the top-$K$ \emph{positive}-$\Delta$ experts, instead of suppressing $\Amin(q)$, applying a positive router bias of equal magnitude. Table~\ref{tab:negresult} shows this collapses: average safety drops to $0\%$ and generation perplexity under Llama-3.1-8B jumps from $12.0$ on \textit{Original} to $>10^{3}$, while \name's suppression variant stays at $12.3$. The order-of-magnitude jump (rather than the absolute number, which depends on the cross-model scorer) is the relevant signal. $\Apos(q)$ does not form a coherent ``safe-expert pool''; its members are merely the experts whose upweighting most depresses the refusal score in a small neighborhood of the current routing, and pushing them up interpolates toward generation collapse rather than refusal. \name's success is therefore tied to suppressing $\Amin(q)$ specifically, not to symmetric routing intervention.
