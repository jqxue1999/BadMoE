...
\end{align}
$\lambda(q)$ is the modulation signal applied per query, while $s(q)$ is the source from which the per-expert identification gradient is taken in the next subsection. We set $\ell^*$ to maximize $\lambda$'s harmful-vs-benign AUROC, since identification is robust across mid-layers but $\lambda$ is not (Appendix~\ref{app:layer_ablation}).
