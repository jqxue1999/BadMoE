...
$b_{\ell,e}(q)$ is computed once per query, at the prompt's last position $T(q)$, before generation begins. A single forward through layers $\{0,\dots,\ell^*\}$ yields the probe state $h(q) = \mathbf{h}_{\ell^*, T(q)}(q)$, from which $\lambda(q)$ and $s(q)$ are read by Equations~\ref{eq:lambda}--\ref{eq:srefuse}; one backward through the same layers yields all $\Delta_{\ell,e}(q)$ simultaneously; Equations~\ref{eq:Aminus} and \ref{eq:bias} then assemble $b_{\ell,e}(q)$.

Once assembled, $b_{\ell,e}(q)$ is \emph{held constant} for the entire generation. A forward hook on each MoE gate module adds the same $b_{\ell,e}(q)$ to the router logits before softmax at every layer and every generated token. The router itself remains per-token: at each generated position $t$, the model recomputes $z_{\ell,t}$ from the running residual and reselects the top-$k$, so different tokens of the same query can still recruit different experts; only the additive logit offset $b_{\ell,e}(q)$ is shared across positions of the query. Model weights are never touched.
