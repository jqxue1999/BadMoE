...
\textcolor{red}{Qwen1.5-MoE-A2.7B-Chat} & \textcolor{red}{$24$} & \textcolor{red}{$60$} & \textcolor{red}{$4$} & \textcolor{red}{$2048$} & \textcolor{red}{$14$} & \textcolor{red}{$0.583$} & $25$ \\
\textcolor{red}{Mixtral-8x7B-Instruct-v0.1} & \textcolor{red}{$32$} & \textcolor{red}{$8$}  & \textcolor{red}{$2$} & \textcolor{red}{$4096$} & \textcolor{red}{$20$} & \textcolor{red}{$0.625$} & \textcolor{red}{$4$} \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Domain Labelling Protocol}
\label{app:domain-labelling}

The five sensitive-content domains used in Table~\ref{tab:main} (drugs, cybercrime, violence, financial, sexual) are obtained by coarsening the JailbreakBench taxonomy. Each of the $100$ JBB harmful goals is labelled once with a Qwen2.5-14B-Instruct classification prompt, asking the judge to assign a single domain from the five-class set. The judge sees only the goal text, not the attack wrapper, so each goal receives one label that is shared across all six attack variants of that goal. Per-domain safety in Table~\ref{tab:main} averages the safe rate over (goals in domain) $\times$ (six attacks). The classification prompt and goal-to-domain mapping are released with the code release. Class balance is approximate: drugs $24$, cybercrime $26$, violence $19$, financial $14$, sexual $17$ (one goal can be re-labelled into the closest class when the original JBB category did not match any of the five).