...
Both challenges have visible consequences at deployment. Figure~\ref{fig:motivation}~(c) illustrates them simultaneously by plotting refusal rate on risky inputs against quality on benign inputs. The static design~\citep{fayyaz2025steermoe} lands in the lower-left region: identification fails per-query, refusing too few risky inputs, and modulation fails per-query, refusing too many benign ones. In numbers, a static design refuses $14$ out of $100$ benign-lookalikes on XSTest while answering $68$ out of $100$ risky inputs, because it cannot tell which experts to suppress nor how strongly to push the bias on a given query. The following is a concrete example of how it fails on both fronts.
